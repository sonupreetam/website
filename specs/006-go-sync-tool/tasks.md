...
- [x] T049 [P] [US2] Add `PeribolosConfig` types in `cmd/sync-content/config.go`: `PeribolosConfig` struct with `Orgs map[string]PeribolosOrg`, `PeribolosOrg` with `Repos map[string]PeribolosRepo`, `PeribolosRepo` with `Description string` and `DefaultBranch string` fields. Parsed by `github.com/goccy/go-yaml` (no new dependency). *(Done — types added to `config.go`.)*
