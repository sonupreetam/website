...
- [ ] T009 [P] [US3] [DEFERRED] Verify config file transforms: check config-declared files at their `dest` paths have correct `inject_frontmatter` (YAML frontmatter with title, description, weight), `rewrite_links` (relative links converted to absolute GitHub URLs), and `strip_badges` (CI badge lines removed) per `sync-config.yaml` transform declarations. *Blocked — `sync-config.yaml` currently has `sources: []`. Unblocked when sources are declared. Code paths covered by unit tests (`TestInjectFrontmatter`, `TestStripBadges`, `TestRewriteRelativeLinks`).*
...
- [x] T018 [P] Update `CONTRIBUTING.md` with sync tool developer workflow: prerequisites (Go 1.25+), running sync tool locally, CLI flags reference, running tests. Reference `specs/006-go-sync-tool/quickstart.md`. *(Done — Prerequisites, Getting Started, Project Structure, Quick Reference, CI/CD, and PR Checklist sections all updated.)*
...
- [x] T020 Validate `specs/006-go-sync-tool/quickstart.md` steps end-to-end: follow each section (clone, dry-run, write, hugo dev, common tasks, verification) and confirm all commands succeed. This also covers SC-001 (`go mod verify`) and SC-002 (`go build ./cmd/sync-content/`) from the quickstart Verification section. *(Done — `go mod verify` → "all modules verified", `go build` succeeds, `go vet` clean, `go test -race` passes, `hugo --minify --gc` succeeds.)*
...
- **US1 (Phase 2)**: Depends on Setup — validates the core workflow (MVP gate)
- **US2, US3, US4 (Phases 3–5)**: Depend on US1 write step (T004) producing generated content
  - US2, US3, US4 can then proceed in parallel
- **US5 (Phase 6)**: Depends on Setup only — workflow files are independent of content verification
  - Can run in parallel with US2–US4
- **US6 (Phase 7)**: Depends on Setup only — tests don't require generated content
  - Can run in parallel with US2–US5
- **Hardening (Phase 8)**: Depends on US6 (Phase 7) — hardening builds on the existing test foundation
  - T028–T036 (implementation) can start after T017 passes (race-free baseline)
  - T037 (hardening tests) depends on T028, T029, T030
  - Tier 2 and Tier 3 tasks are all parallelizable with each other
- **Polish (Phase 9)**: Depends on Hardening — `go vet` / `gofmt` (T019) should run after all code changes
- **Remediation (Phase 10)**: T022 done. T024 done. T025 done. T026a done. T026b done. T027 done.
- **Content Approval Gate (Phase 11)**: Depends on core tool (Phase 2) and deploy workflow (T013). T038–T041 modify sync tool code. T042–T044 modify workflow files. T045–T046 are tests (depend on T038–T041). T047 is a bootstrap file (no deps).
- **Content Transform Improvements (Phase 13)**: Depends on Hardening (Phase 8) for test baseline. T055–T058 are parallel (different functions in `transform.go` and `hugo.go`). T061 (pipeline integration) depends on T055–T058. T060 (render hook) is independent. T062 (docs) depends on all prior Phase 13 tasks.

### User Story Dependencies

- **US1 (P1)**: Setup → US1 — No dependencies on other stories
- **US2 (P1)**: Setup → US1 (T004) → US2 — Needs synced content to verify
- **US3 (P1)**: Setup → US1 (T004) → US3 — Needs synced content to verify
- **US4 (P2)**: Setup → US1 (T004) → US4 — Needs first sync run to compare against
- **US5 (P2)**: Setup → US5 — Workflow files are independent
- **US6 (P3)**: Setup → US6 — Tests are independent of site content
- **Hardening**: US6 (T017) → Hardening — Code changes need race-free baseline first
- **US7 (P2)**: Setup → US5 (T013) → US7 — Lockfile and workflows build on existing deploy pipeline

### Within Each User Story

- Infrastructure tasks before verification tasks
- Verification tasks are sequential within a story (build on previous results)
- All stories are independently testable after their prerequisites are met

### Within Hardening Phase

- T028 (path traversal), T029 (ctx-aware retry), T030 (stale cleanup) can run in parallel — different code regions
- T031–T036 can all run in parallel — independent code changes in different functions
- T037 (hardening tests) depends on T028, T029, T030 completing first — tests verify those fixes

### Parallel Opportunities

- T006 and T007 can run in parallel (different verification targets)
- T008 and T009 can run in parallel (different verification targets)
- T013 and T014 can run in parallel (different workflow files)
- T028, T029, T030 can run in parallel (different code regions in `cmd/sync-content/`)
- T031, T032, T033, T034, T035, T036 can all run in parallel (independent functions)
- T018 and T019 can run in parallel (different concerns)
- After Setup: US5 and US6 can start in parallel with US2–US4
- T038, T039, T040, T041 can run in parallel (different source files in `cmd/sync-content/`)
- T042, T043, T044 can run in parallel (different workflow files)
- T045, T046 depend on T038–T041 (tests verify those code changes)
...
```text
# Content verification track (sequential):
T003 → T004 → T005 (US1 MVP)
  → T006 + T007 (US2, parallel)
  → T008 + T009 (US3, parallel, deferred)
  → T010 (US4)

# CI/CD track (parallel with content track):
T013 ✓ + T014 (US5, T014 remaining)
  → T024 (CI output verification, T013 done)

# Testing track (parallel with content track):
T015 ✓ → T016 ✓ → T017 (US6, T017 remaining)

# Hardening track (after T017):
T028 + T029 + T030 (Tier 1, parallel)
  → T037 (hardening tests, after Tier 1)
T031 + T032 + T033 + T034 + T035 + T036 (Tier 2–3, parallel with Tier 1)

# Remediation track (can start in parallel with content track):
T022 ✓
  → T025 (verify landing page, after T004)
T023 ✓
T026a ✓ → T026b ✓ (sidebar verification, after T004)
T027 (constitution update, after merge)

# Content approval gate track (after T013):
T038 + T039 + T040 + T041 (lock code + ref threading, parallel)
  → T045 + T046 (lock and ref tests, after code changes)
T042 + T043 + T044 (workflow files, parallel with code track)
T047 (bootstrap lockfile, no deps)
```
...
3. **STOP and VALIDATE**: Site builds and displays synced content locally
4. This proves the ported tool integrates correctly with the Hugo site

### Incremental Delivery

1. Setup + US1 → Local preview works (MVP!)
2. Add US2 + US3 → Org discovery and config overlay verified
3. Add US5 → CI/CD pipeline automated
4. Add US6 → Test coverage and race safety
5. **Hardening → Security patched and defensive coding applied**
6. Content Approval Gate → PR-gated content sync operational (US7)
7. Polish → Documentation and final validation

### Parallel Strategy

With the content and CI tracks running in parallel:
- **Track A**: US1 → US2/US3/US4 (content verification)
- **Track B**: US5 (CI/CD workflows)
- **Track C**: US6 → Hardening (unit tests then security/quality fixes)
- **Final**: Polish after all tracks converge

---

## Audit Findings Traceability

The following table maps code audit findings to their corresponding tasks. Findings were categorized by severity and assigned to the appropriate phase.

| # | Finding | Severity | Task | Phase |
|---|---------|----------|------|-------|
| 1 | Path traversal via config `dest` | High | T028 | 8 (Hardening) |
| 2 | Unbounded error body read | Medium | T031 | 8 (Hardening) |
| 3 | URL construction without escaping | Medium | T032 | 8 (Hardening) |
| 4 | Frontmatter parser fragility | Low | — | Deferred (not a bug at current scale) |
| 5 | Rate-limit sleep ignores context | Medium | T029 | 8 (Hardening) |
| 6 | Dry-run skips card building | Low | T033 | 8 (Hardening) |
| 7 | Stale cleanup misses sub-pages (manifest-based `cleanOrphanedFiles` now handles) | Medium | T030 | 8 (Hardening) |
| 8 | Duplicated ProjectCard builder | Low | T034 | 8 (Hardening) |
| 9 | Dead branch fallback code | Low | T035 | 8 (Hardening) |
| 10 | Repeated mutex pattern | Low | — | Deferred (style, not a bug) |
| 11 | Serial recursive API calls | Latent | — | Deferred (meets 60s target) |
| 12 | No HTTP connection pooling tuning | Low | — | Deferred (10 repos, negligible) |
| 13 | Redundant README fetch in discovery | Low | — | N/A (discovery mode removed in T054) |
| 14 | Hardcoded GitHub API URL | Design | — | Deferred (future feature) |
| 15 | Public-only repo listing | Design | — | Deferred (future feature) |
| 16 | Hardcoded exclude list | Medium | T036 | 8 (Hardening) |
| 17 | No log level control | Design | — | Deferred (future feature) |
| 18 | No config schema version | Design | — | Deferred (future feature) |

**Deferred items** are tracked here for visibility. They are not bugs at current scale/scope and can be addressed in a follow-up feature if the tool's usage grows.

---

## Notes

- Phases 1–13 implementation tasks are complete (T001–T062 except T008/T009 deferred)
- Phase 12 (T048–T054) implemented governance-driven repo listing via peribolos.yaml, removed discovery mode and `--discover` flag as redundant, tightened `--repo` to validate against peribolos, removed dead `listOrgRepos` code
- Phase 13 (T055–T062) added content transform improvements: heading level shifting, acronym-aware Title Case with ALL CAPS normalisation, duplicate H1 removal, Hugo render heading hook, `knownAcronyms` map, `formatRepoTitle`/`linkTitle` frontmatter
- Current codebase: 10 source files (~2,100 lines), 10 test files (~2,300 lines, 57 functions)
- 15 of 16 success criteria directly verified; SC-006 deferred but code-path covered by unit tests (`TestSyncConfigSource`, `TestProcessRepo`). SC-016 verified post-merge
- Hardening phase (T028–T037) addressed all 10 in-scope audit findings with 7 new test functions
- Content approval gate (T038–T047) implements PR-gated content sync per Constitution XV
- T027 (constitution sync) completed — memory file updated to v1.5.0
